    ...
    <div className="flex flex-col h-full relative overflow-hidden rounded-md">
          ...
          <div className="p-4 flex justify-between items-center border-b border-gray-500">
            ...
            {activeSubject?.isclaimedbyadmin && (
              <button
                ...
                disabled={isResolved}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm ${isResolved
              ...
              return (
                ...
                >
                  <div className="flex flex-col max-w-[65%]">
                    <div
                      className={`px-4 py-2 rounded-2xl shadow-md text-sm break-words whitespace-pre-wrap ${msg.sender === "admin" ? "bg-[#0D132D] text-white" : "bg-gray-100 text-gray-900"
                        }`}
                    >
                      {/* reply preview */}
                      {msg.replyId && (
                    ...
                    {/* Message time */}
                    <p
                      className={`text-[10px] mt-1 ${msg.sender === "admin" ? "text-gray-500 self-end" : "text-gray-500 self-start"
                        }`}
                    >
                      {formatTime(msg.time)}
                    </p>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2 sm:gap-3 text-gray-600 shrink-0">
            <RiEmojiStickerLine
              className={`text-lg sm:text-xl ${isMobile &&
                !activeSubject || !activeSubject?.isclaimedbyadmin || isResolved
            ...
            <RiImageLine
              className={`text-lg sm:text-xl ${isMobile &&
                !activeSubject || !activeSubject?.isclaimedbyadmin || isResolved ? "opacity-40 cursor-not-allowed" : "cursor-pointer"
                }`}
              onClick={() => {
                if (!activeSubject || !activeSubject?.isclaimedbyadmin || isResolved) return;
          ...
          <motion.button
            whileTap={{ scale: 0.9 }}
            disabled={!activeSubject || !activeSubject?.isclaimedbyadmin || isResolved}
            onClick={handleSend}
