...
const CampaignStep3 = ({ data = {}, onNext, onBack }) => {
    ...
    const newErrors = {
      ...
      applicationenddate:
        ...
        (formData.applicationstartdate &&
          dayjs(formData.applicationenddate).isBefore(
            dayjs(formData.applicationstartdate)
          )),
  ...
  const hasFieldErrors = Object.entries(newErrors).some(([key, val]) => {
    if (key === "milestones") return false;
    return val;
  });

  const hasMilestoneError = milestoneErrors.some(
    (m) => m.description || m.amount || m.enddate
  );

  if (hasFieldErrors || hasMilestoneError) return;

  // check total milestone amount <= budget
...
if (hasFieldErrors || hasMilestoneError) return;
    ...
    const payload = {
      ...
      applicationstartdate:
        formData.applicationstartdate?.format("DD-MM-YYYY") || null,
      applicationenddate:
        formData.applicationenddate?.format("DD-MM-YYYY") || null,
  ...
  return (
    <div className="bg-white p-6 rounded-2xl">
      ...
      <label className="font-semibold block mb-2 flex items-center gap-2 mt-6">
        ...
        <div className="relative group">
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block w-72 bg-gray-900 text-white text-xs rounded px-2 py-1 z-10">
            Application Start and End Dates define when influencers can apply.
            These must be before the campaign start date and cannot be in the
            past.
          </div>
        </div>
      </label>
          ...
          disabledDate={
            (current) =>
              ...
              (current < dayjs().startOf("day") || // no past
                (formData.startDate &&
                  current >= dayjs(formData.startDate).startOf("day"))) // must be before campaign start
          }
          ...
          disabledDate={
            (current) =>
              ...
              (current < dayjs().startOf("day") || // no past
                (formData.applicationStartDate &&
                  current <
                    dayjs(formData.applicationStartDate).startOf("day")) || // must be after app start
                (formData.startDate &&
                  current >= dayjs(formData.startDate).startOf("day"))) // must be before campaign start
          }
      ...
      {errors.applicationstartdate && (
        <p className="text-red-500 text-sm mt-1">
          Please select an application start date
        </p>
      )}
      {errors.applicationenddate && (
        <p className="text-red-500 text-sm mt-1">
          Please select a valid application end date
        </p>
      )}
            ...
            <div className="md:col-span-4">
              <DatePicker
                ...
                disabledDate={(current) => {
                  ...
                  const isBeforeCampaignStart = current.isBefore(
                    campaignStart,
                    "day"
                  );
                  const isAfterCampaignEnd = current.isAfter(
                    campaignEnd,
                    "day"
                  );
      ...
      <button
        ...
        onClick={() => {
          ...
          const isIncomplete =
            !last.description || !last.amount || !last.enddate;
