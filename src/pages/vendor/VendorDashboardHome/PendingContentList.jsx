...
const PendingContentList = () => {
    ...
    const getPendingContentList = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get('/vendor/dashboard/pending-content-list', {
    ...
    // Render providers + content types
                        ...
                        <div className="flex flex-wrap gap-1">
                            {provider.contenttypes?.map((ct) => (
                                <span
                                    ...
                                    className="bg-red-100 text-red-900 rounded px-1"
                                >
        ...
        <div className="bg-white p-6 rounded-2xl w-full overflow-x-auto mt-4">
            ...
            {loading ? (
                ...
                <Table
                    ...
                    columns={columns}
                    rowKey={(record) =>
                        `${record.campaignid}-${record.influencerid}`
                    }
                    ...
                    onRow={(record) => ({
                        onClick: () => {
                            navigate(
                                `/vendor-dashboard/vendor-campaign/campaignDetails/${record.campaignid}`
                            );
                        },
